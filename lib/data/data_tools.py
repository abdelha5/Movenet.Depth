...
class TensorDataset(Dataset):
    ...
    def __getitem__(self, index):
        ...
        # def get_depth(json_file_path, target_key):
        ...
        if(np.shape(depth) != (1,192,192)):
                print("Depth shape: ", np.shape(depth), " index: ", index)
        ...
        #     depths = json.load(f)  
        # depth = depths[item["img_name"]]
        #depth = get_depth(depths_path, item["img_name"])
        ...
        if self.data_aug is not None:
            ...
            with torch.no_grad(): 
                    ...
                    depth = [depth]
            if(np.shape(depth) != (1,192,192)):
                print("Depth Aug shape: ", np.shape(depth), " index: ", index)
        ...
        #print("depth1: ", np.shape(depth))
        depth = np.array(depth, dtype=np.float32)
        #depth = np.transpose(depth, axes=[2,0,1])
        if(np.shape(img) != (3,192,192)):
                print("IMG shape: ", np.shape(img), " index: ", index)
        
        img = np.concatenate((img, depth))
